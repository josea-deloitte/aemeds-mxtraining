...
Cards block estándar del boilerplate de AEM EDS. Transforma cada fila del bloque en un item de una lista (`<ul>` / `<li>`) que se muestra en una cuadrícula responsiva. Cada tarjeta puede contener una imagen y un cuerpo de texto.

## 1. Authoring Contract

Cada **fila** del bloque representa una tarjeta. Dentro de cada fila, cada **celda** se convierte en una parte de la tarjeta:

- Una celda cuyo único contenido es una imagen (`picture`) se marca como `cards-card-image`.
- Cualquier otra celda se marca como `cards-card-body` (título, texto, links, etc.).

### Estructura Conceptual

```text
| cards | |
| [imagen] | ### Título de la tarjeta            |
...
- La detección de imagen es literal: la celda debe tener **un único hijo** que contenga un `<picture>` para clasificarse como `cards-card-image`.
- Las imágenes se reemplazan por versiones optimizadas con `createOptimizedPicture` (ancho de 750px, sin `eager`).

## 2. CSS Customization

`cards.css` no define variables CSS propias. Consume la variable global:
